...
/**
 * Send a webform registration to Zoom meeting upon submission.
 *
 * @WebformHandler(
 *   id = "zoom_api_meeting",
 *   label = @Translation("Send Meeting Registration to Zoom"),
 *   category = @Translation("Web Services"),
 *   description = @Translation("Sends a webform registration to Zoom Meeting upon submission."),
...
class ZoomMeetingRegistrationWebformHandler extends WebformHandlerBase {
  ...
  public function preSave(WebformSubmissionInterface $webform_submission) {
    ...
    $meetingId = $zoom_config->get('obw_zoom_sdk_meeting_registrants_id');
    ...
    curl_close($ch);
    if (!$response) {      
      \Drupal::logger('zoom_meeting_error')->warning('<pre><code>' . print_r(json_decode($err), TRUE) . '</code></pre>');
    }     
    \Drupal::logger('zoom_meeting_success')->warning('<pre><code>' . print_r(json_decode($response), TRUE) . '</code></pre>');
