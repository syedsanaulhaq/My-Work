 ...
 *
 * @WebformHandler(
 *   id = "remote_post_custom",
 *   label = @Translation("Remote post custom"),
 *   category = @Translation("OBW Custom"),
 *   description = @Translation("Posts webform submissions to a queue then
 *   using cronjob to sync AC."), cardinality =
 ...
 * )
 */
class RemotePostHandlerCustom extends WebformHandlerBase {
  ...
  public function postSave(WebformSubmissionInterface $webform_submission, $update = TRUE) {
    ...
    $queue_sync_ac = $queue_factory->get('sync_user_to_ac_queue_worker');
    $queue_sync_ac->createItem($item);
  }
  ...
  protected function getRequestData($state, WebformSubmissionInterface $webform_submission) {
    ...
    if ($this->configuration && isset($this->configuration['excluded_data']) && is_array($this->configuration['excluded_data'])) {
    ...
    /************************/
    //TODO: create options for active campaign
    $ac_fields = [];
    if ($webform_options = WebformOptions::load('obw_ac_sync_tags')) {
      $ac_fields = $webform_options->getOptions() ?: [];
    }

    $is_sync_field = isset($data['field']) && ($data['field'] == 1);
    if ($is_sync_field && !empty($ac_fields)) {
      foreach ($data as $key => $value) {
        if (isset($ac_fields[trim($key)]) && !empty($value)) {
          unset($data[$key]);
          if (is_array($value)) {
            $value = implode('||', $value);
          }
          ...
          if (in_array($key, $convert_field_list)) {
            $value = array_map('trim', explode(",", $value));
            $value = implode('||', $value);
          }
          $data[$ac_fields[trim($key)]] = $value;
        }
      }
    }
    $is_sync_tags = isset($data['sync_tags']) && ($data['sync_tags'] == 1);
    if ($is_sync_tags && !empty($ac_fields)) {
      foreach ($data as $key => $value) {
        if (isset($ac_fields[trim($value)]) && !empty($value)) {
          unset($data[$key]);
          $data['tags'] = $ac_fields[trim($value)];
        }
      }
    }
    /*************************/
  ...
  protected function isDraftEnabled() {
    return $this->isResultsEnabled() && ($this->getWebform()
  ...
  protected function isConvertEnabled() {
    return $this->isDraftEnabled() && ($this->getWebform()
  ...
  protected function debug($message, $state, $request_url, $request_method, $request_type, $request_options, ResponseInterface $response = NULL, $type = 'warning') {
    ...
    $this->messenger()->addMessage(\Drupal::service('renderer')
  ...
  protected function handleError($state, $message, $request_url, $request_method, $request_type, $request_options, $response) {
    ...
    if ($custom_response_message = $this->getCustomResponseMessage($response)) {
      ...
      $this->messenger()->addError(\Drupal::service('renderer')
